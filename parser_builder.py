...
class ParserBuilder:
    def __init__(self):
        ...
        self.cfg: Dict[int, Tuple[str, List[str]]] = dict()
        self.cfg_group: Dict[str, List[str | lexer.TokenType]] = dict()
    ...
    def build_first_set(self) -> Dict[str, Set[str]]:
        ...
        while expand_queue:
            ...
            if expandable is False:
                ...
                del expand_queue[0]
                # print(f'pass {target!r}')
                continue
            else:
                print(f'expand {target!r}')
    ...
    def build_follow_set(self) -> Dict[str, Set[str]]:
        ...
        assert 'select-query' in expand_queue, expand_queue
    ...
    def assign_cfg(self, left, right):
    ...
    def build(self, cfg: List[Tuple[str, List]]):
        ignite_item = 'S', ['query', lexer.TokenType.EOF]
            ...
            if len(state.reduces) > 1:
                for r1 in state.reduces:
                    ...
                    for r2 in state.reduces:
                        ...
                        if intersect:
                            raise ValueError(
                                f'Reduce duplicated{intersect} [{r1},{r2}]\n'
    ...
    def exist_state(self, lhs: State) -> Optional[int]:
        for rhs in self.states.values():
            if len(lhs.items) != len(rhs.items):
                continue
            matched = True
            for k, left_item in enumerate(lhs.items):
                right_item = rhs.items[k]
